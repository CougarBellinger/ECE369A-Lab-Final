...
// assignment.
//
// Create a 1K memory, for which we need 10 bits.  In order to 
...
//
// if 'MemRead' is 1,
//      'ReadData' is the value of memory location Address[11:2] 
// otherwise,
//      it is 0x00000000. The reading of memory is not 
//      clocked.
//  
//
...
module DataMemory(Address, WriteData, Clk, MemWrite, MemRead, ReadData); 
    ...
    reg [31:0] Memory [0:131071]; // 2^17 32-bit elements

    always @ (negedge Clk) begin
        if(MemRead) 
            ReadData <= Memory[Address[19:2]];    // Read data from memory at input address
        else        
            ReadData <= 0;                        // If MemRead = 0, set read data to 0

        if(MemWrite) 
            Memory[Address[19:2]] <= WriteData;   // Write data into memory at input address 
    end
